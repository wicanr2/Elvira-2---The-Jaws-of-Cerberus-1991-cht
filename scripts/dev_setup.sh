#!/bin/bash
# dev-setup 接續包：從零把《艾薇拉 II - 恐怖劇場》繁中化開發環境架起來。
...
mkdir -p run_game
for f in elvira2_zh16.dcjk elvira2_zh24.dcjk elvira2_zh.tab; do
  [ -f "run_game/$f" ] || cp "fonts/$f" "run_game/$f"
done
if [ ! -f run_game/gamepc ]; then
  cat <<'MSG'

  ⚠ run_game/ 尚無遊戲原檔（gamepc、tables01…、text01…、icon.dat、stripped.txt、start 等）。
    請把你合法擁有的 Elvira 2 (Floppy/DOS) 全部檔案(檔名為小寫)複製進 run_game/ 後即可執行：
