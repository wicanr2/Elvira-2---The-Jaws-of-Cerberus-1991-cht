...
# 產出: dist-all/Elvira2-CHT-FULL-win64.zip
set -e
...
docker run --rm -v "$PROJ:/w" -w /w debian:bookworm-slim bash -c '
  ...
  ls -lh scummvm.exe
  STAGE=/w/dist-all/Elvira2-CHT-win64; rm -rf $STAGE; OUT=$STAGE; mkdir -p $OUT/data $OUT/game $OUT/saves
  ...
  cd /w/dist-all; rm -f Elvira2-CHT-FULL-win64.zip
  ( cd /w/dist-all && zip -rq Elvira2-CHT-FULL-win64.zip Elvira2-CHT-win64 )
  rm -rf $STAGE
  echo "=== 產出 ==="; ls -lh /w/dist-all/Elvira2-CHT-FULL-win64.zip
