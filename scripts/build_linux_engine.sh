...
# 使用者自備 Elvira 2 原檔(檔名小寫)放進 game/ 即可玩。產物: dist-all/Elvira2-CHT-linux-x86_64.tar.gz
set -e
...
docker run --rm -v "$PROJ:/w" -w /w agos-build bash -c '
  set -e
  OUT=/tmp/Elvira2-CHT-linux-x86_64; rm -rf "$OUT"
  ...
  # 繁中字型(GPL 衍生，可散佈)
  cp run_game/elvira2_zh16.dcjk run_game/elvira2_zh24.dcjk run_game/elvira2_zh.tab "$OUT/game/"
  # 啟動器
  cat > "$OUT/play-elvira2.sh" <<"EOF"
#!/bin/bash
...
EOF
  chmod +x "$OUT/play-elvira2.sh"
  cp docs/如何開始遊玩.txt "$OUT/README.txt" 2>/dev/null || true
  ( cd /tmp && tar czf /w/dist-all/Elvira2-CHT-linux-x86_64.tar.gz "Elvira2-CHT-linux-x86_64" )
  chown -R '"$(id -u):$(id -g)"' /w/dist-all 2>/dev/null || true
  echo "=== 產物 ==="; ls -lh /w/dist-all/Elvira2-CHT-linux-x86_64.tar.gz
'
