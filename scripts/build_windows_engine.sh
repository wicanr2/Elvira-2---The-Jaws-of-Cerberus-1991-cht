...
# 需先有 dist-all/Elvira2-CHT-FULL-win64.zip(build_windows.sh 產)。
# 產物: dist-all/Elvira2-CHT-windows-x64.zip
set -e
PROJ="/home/anr2/scummvm/elvira_2_cht/workplace"
FULL="$PROJ/dist-all/Elvira2-CHT-FULL-win64.zip"
...
docker run --rm -v "$PROJ:/w" -w /w agos-build bash -c '
  ...
  cd "$T"; unzip -q /w/dist-all/Elvira2-CHT-FULL-win64.zip
  SRC=Elvira2-CHT-win64
  OUT=Elvira2-CHT-windows-x64; rm -rf "$OUT"; mkdir -p "$OUT/data" "$OUT/game" "$OUT/saves"
  # 引擎 + DLL + 資料(排除 ROM)
  cp "$SRC"/*.exe "$SRC"/*.dll "$OUT/"
  for f in "$SRC"/data/*; do case "$(basename "$f")" in *.ROM) ;; *) cp "$f" "$OUT/data/";; esac; done
  # game/ 只留繁中字型(不含遊戲/ROM)
  cp "$SRC"/game/elvira2_zh16.dcjk "$SRC"/game/elvira2_zh24.dcjk "$SRC"/game/elvira2_zh.tab "$OUT/game/"
  # 啟動器 + README
  printf "@echo off\r\ncd /d \"%%~dp0\"\r\nif not exist saves mkdir saves\r\nscummvm.exe -p game --themepath=data --extrapath=game --music-driver=mt32 --auto-detect --savepath=saves\r\npause\r\n" > "$OUT/play-elvira2.bat"
  cp /w/docs/如何開始遊玩.txt "$OUT/README.txt" 2>/dev/null || true
  zip -rq /w/dist-all/Elvira2-CHT-windows-x64.zip "$OUT"
  chown -R '"$(id -u):$(id -g)"' /w/dist-all 2>/dev/null || true
  echo "=== 產物 ==="; ls -lh /w/dist-all/Elvira2-CHT-windows-x64.zip
