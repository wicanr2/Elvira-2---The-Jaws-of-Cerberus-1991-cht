...
# 艾薇拉II-恐怖劇場 (Elvira 2 - The Jaws of Cerberus (1991)）中文化
 - 目標：中文化當年的經典
 - 引擎：上網查詢
 - 中文化僅放 scummvm patch
...
# github repo（只放 patch-only）
 - https://github.com/wicanr2/Elvira-2---The-Jaws-of-Cerberus-1991-cht.git
 - 手冊與攻略也需要整理放到 github repo 
 - README.md 除了放技術資料外, 也要把遊戲介紹弄的引人入勝 (盡量引用攻略與手冊經典段落)
...
| macOS x86_64(Rosetta)弧連結 `found architecture arm64, required x86_64` / undefined symbols(libjpeg…) | configure **自動偵測到 runner 上 brew 的 `.a`（arm64-only）**連進 x86_64 弧 | **LEAN_FLAGS 全 disable 所有外部可選庫**(旗標撈自該版 configure，別照抄新版)，只留內建 mt32emu/nuked-opl |
| ScummVM configure 旗標照抄新版 → `unrecognized option`(如 `--disable-system-printing`) | 旗標跨版本不同 | **`grep -oE '\-\-disable-[a-z0-9-]+' configure`** 撈當前版有效集；Linux 端先 configure 驗一次 |
...
- **[HARD] LEAN_FLAGS 全 disable 所有外部可選庫**：否則 configure 撿到 runner 上 **brew 的 arm64-only `.a`（libjpeg 等）**，x86_64(Rosetta)弧連結時 `found architecture arm64, required x86_64` → undefined symbols。旗標**撈自該版 configure**（`grep -oE '\-\-disable-[a-z0-9-]+' configure`），別照抄新版（`--disable-system-printing` 在 2.9.1 不存在）。保留 `mt32emu`/`nuked-opl`(內建)。
